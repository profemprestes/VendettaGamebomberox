...
ALTER TABLE public.configuracion_habitacion ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.configuracion_entrenamiento ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.configuracion_tropa ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE public.cola_misiones ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.mensaje ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.informe_batalla ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.informe_espionaje ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.familia ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.miembro_familia ENABLE ROW LEVEL SECURITY;

-- Políticas Detalladas
-- public.usuario
CREATE POLICY "lectura_publica" ON public.usuario FOR SELECT USING (true);
CREATE POLICY "modificacion_propia" ON public.usuario FOR UPDATE USING (auth.uid() = id);

-- public.propiedad
CREATE POLICY "lectura_publica" ON public.propiedad FOR SELECT USING (true);
CREATE POLICY "modificacion_dueno" ON public.propiedad FOR UPDATE USING (usuario_id = auth.uid());

-- public.configuracion_*
CREATE POLICY "lectura_global" ON public.configuracion_habitacion FOR SELECT USING (true);
CREATE POLICY "lectura_global" ON public.configuracion_entrenamiento FOR SELECT USING (true);
CREATE POLICY "lectura_global" ON public.configuracion_tropa FOR SELECT USING (true);

-- public.cola_*
CREATE POLICY "ver_propias_construccion" ON public.cola_construccion FOR SELECT USING (EXISTS (SELECT 1 FROM propiedad WHERE id = cola_construccion.propiedad_id AND usuario_id = auth.uid()));
CREATE POLICY "ver_propias_investigacion" ON public.cola_investigacion FOR SELECT USING (usuario_id = auth.uid());
CREATE POLICY "ver_propias_reclutamiento" ON public.cola_reclutamiento FOR SELECT USING (EXISTS (SELECT 1 FROM propiedad WHERE id = cola_reclutamiento.propiedad_id AND usuario_id = auth.uid()));
CREATE POLICY "ver_propias_misiones" ON public.cola_misiones FOR SELECT USING (usuario_id = auth.uid());

-- public.mensaje
CREATE POLICY "ver_mis_mensajes" ON public.mensaje FOR SELECT USING (remitente_id = auth.uid() OR destinatario_id = auth.uid());
CREATE POLICY "enviar_mensaje" ON public.mensaje FOR INSERT WITH CHECK (auth.uid() = remitente_id);

-- public.informe_*
CREATE POLICY "ver_participantes_batalla" ON public.informe_batalla FOR SELECT USING (atacante_id = auth.uid() OR defensor_id = auth.uid());
CREATE POLICY "ver_participantes_espionaje" ON public.informe_espionaje FOR SELECT USING (atacante_id = auth.uid() OR defensor_id = auth.uid());

-- public.familia y miembros
CREATE POLICY "lectura_publica_familia" ON public.familia FOR SELECT USING (true);
CREATE POLICY "lectura_publica_miembros" ON public.miembro_familia FOR SELECT USING (true);
-- Políticas de gestión de líderes (simplificadas, se pueden expandir)
CREATE POLICY "gestion_lideres" ON public.familia FOR UPDATE USING (EXISTS (SELECT 1 FROM miembro_familia WHERE familia_id = id AND usuario_id = auth.uid() AND rol IN ('lider', 'capitan')));


-- Habilitar RLS en tablas restantes
ALTER TABLE public.historial_acceso ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.puntuacion_usuario ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.errores_configuracion ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.requisito_habitacion ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.requisito_entrenamiento ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.tropa_bonus_contrincante ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.habitacion_usuario ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.tropa_propiedad ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.tropa_seguridad_propiedad ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.entrenamiento_usuario ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.cola_eventos_flota ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.invitacion_familia ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.anuncio_familia ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.ataque_entrante ENABLE ROW LEVEL SECURITY;

-- Políticas para tablas restantes
CREATE POLICY "ver_propio_historial" ON public.historial_acceso FOR SELECT USING (usuario_id = auth.uid());
CREATE POLICY "ver_propia_puntuacion" ON public.puntuacion_usuario FOR SELECT USING (usuario_id = auth.uid());
CREATE POLICY "ver_propios_errores" ON public.errores_configuracion FOR SELECT USING (usuario_id = auth.uid());

CREATE POLICY "lectura_global" ON public.requisito_habitacion FOR SELECT USING (true);
CREATE POLICY "lectura_global" ON public.requisito_entrenamiento FOR SELECT USING (true);
CREATE POLICY "lectura_global" ON public.tropa_bonus_contrincante FOR SELECT USING (true);

CREATE POLICY "ver_propias_habitaciones" ON public.habitacion_usuario FOR SELECT USING (EXISTS (SELECT 1 FROM propiedad WHERE id = habitacion_usuario.propiedad_id AND usuario_id = auth.uid()));
CREATE POLICY "ver_propias_tropas" ON public.tropa_propiedad FOR SELECT USING (EXISTS (SELECT 1 FROM propiedad WHERE id = tropa_propiedad.propiedad_id AND usuario_id = auth.uid()));
CREATE POLICY "ver_propias_tropas_seguridad" ON public.tropa_seguridad_propiedad FOR SELECT USING (EXISTS (SELECT 1 FROM propiedad WHERE id = tropa_seguridad_propiedad.propiedad_id AND usuario_id = auth.uid()));
CREATE POLICY "ver_propios_entrenamientos" ON public.entrenamiento_usuario FOR SELECT USING (usuario_id = auth.uid());

CREATE POLICY "ver_propias_invitaciones" ON public.invitacion_familia FOR SELECT USING (usuario_id = auth.uid() OR EXISTS (SELECT 1 FROM miembro_familia WHERE familia_id = invitacion_familia.familia_id AND usuario_id = auth.uid() AND rol IN ('lider', 'capitan')));
CREATE POLICY "ver_anuncios_familia" ON public.anuncio_familia FOR SELECT USING (EXISTS (SELECT 1 FROM miembro_familia WHERE familia_id = anuncio_familia.familia_id AND usuario_id = auth.uid()));
CREATE POLICY "ver_ataques_entrantes" ON public.ataque_entrante FOR SELECT USING (defensor_id = auth.uid());
